...
const Page = ({setCurrentPage, setSelectedAd, login}) => {
    ...
    // }
    const removeAd = (ad) => {
        const index = items.indexOf(ad);
        const removed = (items.slice(0,index)).concat(items.slice(index+1,items.length));
    ...
    const [items,setItems] = useState(new Array(20).fill(
        {
            ...
            expirationDate: new Date('2022-12-20'),
            imagePath: 'ad12389129387',
            linkUrl: "https://www.google.com/"
        })
    );

    const cards = {
        publishedAds: {amount: 10, text: 'Anúncios Publicados'},
    }

    const AdCard = ({ad}) => {
        const adBannerBase46 = getImageData(ad.imagePath);
        return (
            <div className='adCard horizontalAlign'>
                ...
                <div className='horizontalAlign' style={{width: '100%'}}>
                    <img className='adBanner' src={`data:image/jpeg;base64,${adBannerBase46}`} alt={`Banner do anúncio da empresa "${ad.companyName}"`}/>
                </div>
            </div>
        )
    }
